  ...
  async runBacktest(
    ...
    for (const symbol of symbols) {
      ...
      for (let i = startIdx; i < primaryBars.length; i++) {
        ...
        if ((signal === 'BUY' || signal === 'SELL') && i + 1 < primaryBars.length) {
          const nextClose = primaryBars[i + 1].close;
          profitPercent =
            signal === 'BUY'
              ? ((nextClose - currentBar.close) / currentBar.close) * 100
              : ((currentBar.close - nextClose) / currentBar.close) * 100;
    ...
    return {
      totalSignals,
      winCount,
      lossCount: totalSignals - winCount,
