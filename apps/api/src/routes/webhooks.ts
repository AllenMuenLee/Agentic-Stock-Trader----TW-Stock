...
router.post('/line', async (req: Request, res: Response) => {
  ...
  for (const event of events) {
    ...
    if (event.type === 'follow') {
      ...
      await replyLine(event.replyToken, '歡迎使用智股通！\n請發送您的 6 位數綁定碼（大寫英數字）以完成帳號綁定。');
      continue;
    }
    ...
    if (event.type === 'message' && event.message?.type === 'text') {
      ...
      if (/^[0-9A-F]{6}$/.test(text)) {
        ...
        if (user) {
          ...
          await replyLine(event.replyToken, `綁定成功！您的智股通帳號（${user.username}）已與 LINE 綁定，股票訊號將即時推送給您。`);
        } else {
          await replyLine(event.replyToken, '綁定碼無效或已過期，請至智股通設定頁重新取得綁定碼。');
        }
      } else {
        await replyLine(event.replyToken, '請在智股通設定頁面取得 6 位數綁定碼，並將其發送至此。');
      }
    }
  }
